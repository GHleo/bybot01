...
def fhUSDM_initDOWN(lblPriceInDn_):
    ...
    cnfg.positLngDn[0] = round_up(cnfg.balanceLnOnLev / cnfg.costsLn[0],
                                  qtyStep)  # calculate count in first position - truncate quantity because of restrictions at futures rules
    print('fhUSDM_initDOWN() Qty(long)  cnfg.positLngDn[0] = ' + str(cnfg.positLngDn[0]))
    cnfg.balanceLngRatDn[0] = round(cnfg.positLngDn[0] * cnfg.costsLn[0],
                                    2)  # REWRITE balance after truncate quontity !!!!!!
    cnfg.costTP_Long[0] = round(cnfg.costsLn[0] * (1 + float(cnfg.lngTPfirstDn[0]) / 100),
                                cnfg.pricePrc)  # cost first TP
    cnfg.costSL_Long[0] = round(cnfg.costsLn[0] * (1 - float(cnfg.lngSLfirstDn[0]) / 100),
                                cnfg.pricePrc)  # cost first SL !!!!!!!!!!!! TRIGER IN OCO !!!!!!
    ...
    while cnfg.loopItems <= cnfg.trades:
        try:
            ...
            if (positionValue != '0') and (positionValue != ''):
                ...
                print('ml  !!!!!?????? mlastPrice: ' + str(mlastPrice) + '; cnfg.costsLn: ' + str(
                    cnfg.costsLn) + '; cnfg.costsSh: ' + str(cnfg.costsSh))
                diffPercLn = round(
                    (mlastPrice - cnfg.costsLn[cnfg.loopItems - 1]) / cnfg.costsLn[cnfg.loopItems - 1] * 100,
                    2)  # difference of first IN and Current cost for Long
                diffPercDn = round((cnfg.costsSh[cnfg.loopItems - 1] - mlastPrice) / mlastPrice * 100,
                                   4)  # difference of first IN and Current cost for Short
                # ordersInfo2 = ordersInfo["result"]["list"]
                ...
                if cnfg.isDown:
                    ...
                    print('ml diff Short ->' + ' Original price: ' + str(
                        cnfg.costsSh[cnfg.loopItems - 1]) + ' Current price: ' + str(mlastPrice) + ' Diff: ' + str(
                        round(cnfg.costsSh[cnfg.loopItems - 1] - mlastPrice, 2)) + '$' + ' Diff: ' + str(
                        diffPercDn) + '%')
                    print('isUp -> get_execution Sell Order: ' + str(
                        cnfg.session.get_executions(category="linear", orderId=cnfg.orderID_sell, limit=1, )))
                    if (diffPercDn >= tpShortFirst / 2) and (diffPercDn > 0):
                        # print('ml edit Order -> ')
                        shNextPrice = round(mlastPrice * (1 + tpShortFirst / 2 / 100), 2)
                        print('ml edit Short -> mlastPrice: ' + str(mlastPrice) + '; tpShortFirst/2: ' + str(
                            tpShortFirst / 2) + '; shNextPrice: ' + str(shNextPrice))
                        shNextTP = round(cnfg.costTP_Short[cnfg.loopItems - 1] * (1 + (tpShortFirst / 2 / 100)), 2)
                        print('ml edit Short -> cnfg.costTP_Short: ' + str(
                            cnfg.costTP_Short) + '; tpShortFirst/2: ' + str(tpShortFirst / 2) + '; shNextTP: ' + str(
                            shNextTP))
            ...
            if not positionValue and (cnfg.loopItems < cnfg.trades) and (Pnl_ < 0):  # if position close and need make new order
                ...
                if cnfg.isDown:
                    cnfg.pnlTotal += Pnl_
                    cnfg.orderID_sell, cnfg.retMsg_sell = createOrder('Sell', cnfg.levUP, '0',
                                                                      cnfg.costTP_Short[cnfg.loopItems - 1],
                                                                      cnfg.costSL_Short[cnfg.loopItems - 1],
                                                                      cnfg.positSh[cnfg.loopItems - 1], exept_,
                                                                      'MARKET', 0)
                    cnfg.costsSh[cnfg.loopItems - 1] = lastPrice()  # cost of order
                    print('ml next trade!  cnfg.orderID_sell: ' + str(cnfg.orderID_sell) + '; cnfg.retMsg_sell: ' + str(
                        cnfg.retMsg_sell))
                    print('ml next trade!  cnfg.loopItems: ' + str(cnfg.loopItems) + '; cnfg.costTP_Short: ' + str(
                        cnfg.costTP_Short) + '; cnfg.costSL_Short: ' + str(cnfg.costSL_Short))
                    scrMain_.insert(tk.END, '\ncreate Order Sell; sell ID: ' + str(cnfg.orderID_sell) + '; ' + str(
                ...
                if cnfg.isUp:
                    ...
                    cnfg.orderID_buy, cnfg.retMsg_buy = createOrder('Buy', cnfg.levDn, '0',
                                                                    cnfg.costTP_Long[cnfg.loopItems - 1],
                                                                    cnfg.costSL_Long[cnfg.loopItems - 1],
                                                                    cnfg.positLng[cnfg.loopItems - 1], exept_, 'MARKET',
                                                                    0)
...
def createOrder(side_, lev_, prc_, tp_, sl_, qty_, exept_, type_, posIdx_):
    try:
        print('create Order() side_: ' + str(side_) + '; prc_: ' + str(prc_) + '; tp_: ' + str(tp_) + '; sl_: ' + str(
            sl_) + '; qty_: ' + str(qty_) + '; ' + str(dt.now().strftime('%H:%M:%S')))
        if type_ == 'LIMIT':
